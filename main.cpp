...
//[4]    Utils.writeToFile(newbuff, "/home/ilian/test1.mkv", "wb");
        ...
        return a.exec();
#elif defined LISTTEST
    DList* linstance = createList("New list");

    addFront("abc1", linstance);
    addFront("abc2", linstance);
    addFront("abc3", linstance);
    addFront("abc4", linstance);
...
#include "vector.h"
int main(void) {
    Vector* v = newVector(100);

    int i=0;
    while ( i++ < 100000 ) {
        v->callbacks->addElement(v, (int*)i);
    }
    i=0;
    while (i++ < 100000 )
        printf("%d\n", (int*) v->callbacks->getElement(v, i));
}
